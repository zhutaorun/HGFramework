...
public class Lua_InvEquipment : LuaObject {
	...
	static public int Unequip(IntPtr l) {
		try {
			...
			if(matchType(l,argc,2,typeof(InvBaseItem.Slot))){
				InvEquipment self=(InvEquipment)checkSelf(l);
				InvBaseItem.Slot a1;
				checkEnum(l,2,out a1);
				var ret=self.Unequip(a1);
				pushValue(l,ret);
				return 1;
			}
			else if(matchType(l,argc,2,typeof(InvGameItem))){
				InvEquipment self=(InvEquipment)checkSelf(l);
				InvGameItem a1;
				checkType(l,2,out a1);
				var ret=self.Unequip(a1);
	...
	static public int get_equippedItems(IntPtr l) {
		try {
			InvEquipment self=(InvEquipment)checkSelf(l);
			pushValue(l,self.equippedItems);
			return 1;
		}
		catch(Exception e) {
			LuaDLL.luaL_error(l, e.ToString());
	...
	static public void reg(IntPtr l) {
		...
		createTypeMetatable(l,null, typeof(InvEquipment),typeof(UnityEngine.MonoBehaviour));
	}
}
